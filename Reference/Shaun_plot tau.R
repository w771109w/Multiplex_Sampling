...
library(sp)
library(compiler)
install.packages("labelled")  # 安裝 labelled 套件
library(labelled)

install.packages("data.table")  # 安裝 data.table
library(data.table)  # 載入 data.table
...
# read.and.clean.data <- function(country, year, country.dhs, country.gps, age.range=c(13,60)){
  ...
  if(!is.null(data.info)){
    ...
    # GPS data
    gps.data <- read.dbf(file=paste0(directory, country.gps))
...
dhs.data.import <- cmpfun(function(country, data.year=NA, filename, filename.gps,
  ...
  var.labels <- labelled::var_label(dataDHS)
  var.value.labels <- labelled::var_label(dataDHS)
